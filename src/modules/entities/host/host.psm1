function Get-allHostsfromPc {
    ...
    process {
      try {
        $apiEndpoint = "/api/nutanix/v3/hosts/list"
        $apiMethod = "POST"
        $jsonRootPath = "entities"
        $apiKind = 'host'
              
        $hostList = Get-AllPrismCentralObjectsByType `
        ...
        write-host 'Number of Virtual Machines found: ' $hostList.count
      }
      catch {
